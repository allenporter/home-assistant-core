...
async def async_setup_entry(
    ...
    def async_add_select(coordinator: RoborockDataUpdateCoordinator) -> None:
        ...
        if (home_trait := coordinator.properties_api.home) is not None and (
            map_trait := coordinator.properties_api.maps
        ) is not None:
            entities.append(
    ...
    config_entry.async_on_unload(
        coordinators.v1_dispatcher.async_dispatcher_connect(async_add_select)
    )
    async_add_entities(

...
@pytest.mark.parametrize("platforms", [[Platform.SENSOR]])
async def test_zeo_device_unavailable(
    hass: HomeAssistant,
    mock_roborock_entry: MockConfigEntry,
    device_registry: DeviceRegistry,
    ...
    assert devices_with_entities == {
        ...
        # Zeo device is created, however the entities should be unavailable
        "Zeo One",
    }
...
@pytest.mark.parametrize("platforms", [[Platform.SENSOR]])
async def test_dyad_device_unavailable(
    hass: HomeAssistant,
    mock_roborock_entry: MockConfigEntry,
    device_registry: DeviceRegistry,
    entity_registry: EntityRegistry,
    ...
    assert devices_with_entities == {
        ...
        "Roborock S7 2",
        "Roborock S7 2 Dock",
        # Dyad device is created but entities are unavailable
        "Dyad Pro",
        "Zeo One",
        "Roborock Q7",
        # Q10 has no sensor entities
...
@pytest.mark.parametrize("platforms", [[Platform.SENSOR]])
async def test_devices_available(
    hass: HomeAssistant,
    mock_roborock_entry: MockConfigEntry,
    device_registry: DeviceRegistry,
    fake_devices: list[FakeDevice],
) -> None:
    """Exercise the case where devices are available and entities are available."""
    await hass.config_entries.async_setup(mock_roborock_entry.entry_id)
    assert mock_roborock_entry.state is ConfigEntryState.LOADED
    ...
    states = hass.states.get("sensor.zeo_one_countdown")
    assert states is not None
    assert states.state == "0"
    states = hass.states.get("sensor.dyad_pro_total_cleaning_time")
    assert states is not None
    assert states.state == "3.55"
